...
# save.image("SRP151137_plot/list.RData")
# load("SRP151137_plot/list.RData")
...
# save.image("SRP151137_plot/ENCODE_All_Meth.RData")
load("SRP151137_plot/ENCODE_All_MOFA.RData")
...
volcano_data <- SRP151137_CpG_DEG %>%
  mutate(
    chr = SRP151137_CpG_DEG$chr,  # 确保有基因名列
...
ggplot(volcano_data, aes(x = logFC, y = log_pval, colour = direction)) +
  ...
  theme_bw()+
  ggtitle("CpG ENCODE Methlevel DEG Volcano")+
  # 图例
  ...
  xlim(-2, 2) + ylim(0, 15)+
  geom_label_repel(data = top_genes_up, aes(label = chr),
                   size = 4,                           # 设置标签大小
  ...
  geom_label_repel(data = top_genes_down, aes(label = chr),
                   size = 4,                           # 设置标签大小
                   box.padding = unit(0.8, "lines"),   # 设置标签内边距
                   point.padding = unit(0.8, "lines"), # 设置标签与点的距离
                   segment.color = "black",            # 设置标签边界线颜色
...
ggplot_data <- SRP151137_E6.5_diff_data_choose_p
...
ggplot(data = ggplot_data,mapping = aes(x = CpG_rate_diff, y = GpC_rate_diff))+
  ...
  labs(x = "Differential methylation (%)",y = "Differential accessibility (%)",
       title = "E4.5 ENCODE Differential")+
  theme(legend.title=element_blank(),
        # plot.title = element_text(hjust = 0.5,size = 18,face = "bold"),
...
ggplot(CH_SC, aes(x = type, y = CH.value)) +
  ...
  geom_text(aes(label = round(CH.value, 3)),
...
ggplot(CH_SC, aes(x = type, y = SC.value)) +
  geom_bar(stat = "identity", position = position_dodge(0.8), width = 0.7, fill = "#4E79A7")+
  # geom_col(position = "dodge")+
  geom_text(aes(label = round(SC.value, 3)),
            position = position_dodge(0.8),
            vjust = -0.5,
            size = 4) +
  # scale_fill_manual(values = c("CpG.ENCODE" = "#4E79A7", "GpC.ENCODE" = "#F28E2B",
