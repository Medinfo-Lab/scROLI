...
SRP151137_RNA_data_gene <- SRP151137_RNA_data[,c("Geneid","Length","Genename")]
# SRP151137_RNA_data_gene <-  unique(SRP151137_RNA_data_gene, by = c("Geneid", "Length","Genename"))
...
SRP151137_RNA_data_counts_filter <- dedup_dt
...
SRP151137_pbmc <- FindClusters(SRP151137_pbmc,resolution = 0.07)
SRP151137_pbmc <- RunUMAP(SRP151137_pbmc, dims = 1:5,
                          min.dist = 3,spread = 5)

DimPlot(SRP151137_pbmc,reduction = "umap",pt.size = 2.5,group.by = "group1")+
  ggtitle("RNA Group UMAP")+
  theme(plot.title = element_text(hjust = 0.5,size = 20,face = "bold"),
        axis.title.x = element_text(face = "bold",size = 15),
        axis.title.y = element_text(face = "bold",size = 15),
        axis.test.x = element_text(face = "bold",size = 15),
        axis.test.x = element_text(face = "bold",size = 15))+
...
# Idents(SRP151137_pbmc) <- SRP151137_pbmc$seurat_clusters
...
colnames(SRP151137_pbmc_markers)[7] <- "Geneid"
SRP151137_pbmc_markers_result  <- merge(
  x = SRP151137_pbmc_markers,
  y = SRP151137_RNA_data_gene,
  by = "Geneid",
  all.x = TRUE
)
SRP151137_pbmc_markers_result$Geneid2 <- remove_after_last_dot(SRP151137_pbmc_markers_result$Geneid)
# save.image("SRP151137_plot/RNA_markers.RData")
load("SRP151137_plot/RNA_markers.RData")

SRP151137_pbmc_markers_P <- SRP151137_pbmc_markers_result %>%
  filter(p_val < 5e-7)

# SRP151137_pbmc_markers_P_E4.5 <- SRP151137_pbmc_markers_P %>%
#   filter(cluster == "E4.5")
...
volcano_data <- SRP151137_pbmc_markers_P %>%
  mutate(
    gene = SRP151137_pbmc_markers_P$Genename,  # 确保有基因名列
...
#KEGG GO----
enrichment_genes_E4.5 <- SRP151137_pbmc_markers_P %>%
  filter(cluster == "E4.5")

enrichment_genes_E5.5 <- SRP151137_pbmc_markers_P %>%
  filter(cluster == "E5.5")

enrichment_genes_E6.5 <- SRP151137_pbmc_markers_P %>%
  filter(cluster == "E5.5")
...
ego <- enrichGO(gene = SRP151137_pbmc_markers_P$Geneid2,
                keyType = "ENSEMBL",
                OrgDb = org.Mm.eg.db,
                ont = "ALL",        # "BP","MF","CC"或"ALL"
                pAdjustMethod = "BH",         # 校正方法：BH, bonferroni等
